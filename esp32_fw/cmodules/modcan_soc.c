...
//#define TX_PIN 39
    ...
    int speed_kbits = mp_obj_get_int(speed_obj);
    // Calculate the addition and convert to MicroPython object.
    // Initialize configuration structures using macro initializers
    twai_general_config_t g_config = TWAI_GENERAL_CONFIG_DEFAULT((gpio_num_t)tx_pin, (gpio_num_t)rx_pin, TWAI_MODE_LISTEN_ONLY);
    //Look in the api-reference for other speed sets.
    switch (speed_kbits)
              {
                case 100:
...
static mp_obj_t can_read_msg_id(mp_obj_t identifier_obj, mp_obj_t data_byte_number_obj) {
    ...
    if (alerts_triggered & TWAI_ALERT_RX_DATA) {
        ...
        while (twai_receive(&message, 0) == ESP_OK) {
           if (!(message.rtr)) {
              int byte_0 = 0;
              if (identifier == 0) return mp_obj_new_int(message.identifier);
              if (message.identifier == identifier) byte_0 = message.data[data_byte_number];
              soc = byte_0;
...
// Define a Python reference to the function above.
static MP_DEFINE_CONST_FUN_OBJ_1(can_read_msg_id_obj, can_read_msg_id);
